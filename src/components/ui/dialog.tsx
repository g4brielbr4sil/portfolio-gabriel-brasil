...
export function DialogContent({ className, children, fullScreenMobile = true, ...props }: DialogContentProps) {
  return (
    <DialogPortal>
      <DialogPrimitive.Overlay className="fixed inset-0 z-[70] bg-black/78 backdrop-blur-sm data-[state=closed]:animate-[fade-out_180ms_ease] data-[state=open]:animate-[fade-in_220ms_ease]" />
      <DialogPrimitive.Content
        className={cn(
          'fixed left-1/2 top-1/2 z-[80] w-[min(92vw,1400px)] -translate-x-1/2 -translate-y-1/2 overflow-hidden border border-line bg-[#0a0a0a] text-cream shadow-[0_28px_100px_rgba(0,0,0,0.75)] outline-none data-[state=closed]:animate-[fade-out_180ms_ease] data-[state=open]:animate-[fade-in_220ms_ease]',
          fullScreenMobile &&
