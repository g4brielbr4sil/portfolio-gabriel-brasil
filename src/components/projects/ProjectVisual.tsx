...
export default function ProjectVisual({ project, className = '' }: { project: Project; className?: string }) {
  ...
  if (project.slug === 'radar-df') {
    return (
      <div
        className={`relative h-full overflow-hidden bg-[#0b0608] ${className}`}
        role="img"
        aria-label="Mockup conceitual do Radar DF, projeto em desenvolvimento"
      >
        <div className="absolute inset-0 opacity-45 [background-image:linear-gradient(rgba(159,16,42,.14)_1px,transparent_1px),linear-gradient(90deg,rgba(159,16,42,.14)_1px,transparent_1px)] [background-size:28px_28px]" />
        <div className="absolute inset-[8%] overflow-hidden rounded-xl border border-[#9f102a]/35 bg-[#100b0d]/92 shadow-[0_22px_55px_rgba(0,0,0,.5)]">
          <div className="flex h-[19%] items-center border-b border-white/8 px-[4%]">
            <div className="flex gap-1" aria-hidden="true">
              <span className="size-1.5 rounded-full bg-[#720b1d]" />
              <span className="size-1.5 rounded-full bg-[#9f102a]" />
              <span className="size-1.5 rounded-full bg-[#c61a3a]" />
            </div>
            <span className="ml-3 font-mono text-[7px] uppercase tracking-[0.18em] text-white/42">Radar DF · conceito</span>
            <span className="ml-auto rounded-full border border-[#9f102a]/30 px-2 py-0.5 font-mono text-[6px] uppercase tracking-[0.12em] text-[#d06a7e]">Em desenvolvimento</span>
          </div>
          <div className="grid h-[81%] grid-cols-[28%_72%]">
            <div className="border-r border-white/8 p-[10%]">
              {['Oportunidades', 'Currículo', 'Matching', 'Candidaturas'].map((label, index) => (
                <div key={label} className={`mb-[7%] rounded px-2 py-1.5 font-mono text-[6px] uppercase tracking-[0.08em] ${index === 0 ? 'bg-[#9f102a]/18 text-[#dc7b8c]' : 'text-white/28'}`}>
                  {label}
                </div>
              ))}
            </div>
            <div className="p-[5%]">
              <div className="flex items-end justify-between">
                <div>
                  <span className="block font-mono text-[6px] uppercase tracking-[0.15em] text-[#c96c7d]">Painel conceitual</span>
                  <strong className="mt-1 block text-[11px] font-semibold text-white/82 sm:text-sm">Oportunidades organizadas</strong>
                </div>
                <span className="rounded border border-white/10 px-2 py-1 font-mono text-[6px] text-white/38">DF</span>
              </div>
              <div className="mt-[6%] grid gap-2">
                {['Matching em análise', 'Currículo estruturado', 'Candidatura acompanhada'].map((label, index) => (
                  <div key={label} className="flex items-center gap-2 rounded-md border border-white/8 bg-white/[0.025] px-2.5 py-2">
                    <span className={`size-1.5 rounded-full ${index === 0 ? 'bg-[#c61a3a]' : 'bg-white/20'}`} />
                    <span className="font-mono text-[6px] uppercase tracking-[0.08em] text-white/46">{label}</span>
                    <span className="ml-auto h-1 w-[22%] rounded-full bg-[#9f102a]/22" />
                  </div>
                ))}
              </div>
            </div>
